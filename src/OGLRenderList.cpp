...
void OGLRenderList::UpdateBuffer(u32 iCount, UpdateData *aUpdateData, Color *pUpdateColor)
{
    int iVertsPerSprite = (m_kRLT == RLT_LINKED ? 4 : 6);
        ...
        /*glBindBuffer(GL_ARRAY_BUFFER_ARB, m_iVBOVCID);
        float *aVerts = (float *) glMapBufferARB(GL_ARRAY_BUFFER_ARB, GL_WRITE_ONLY_ARB);
        glBindBuffer(GL_ARRAY_BUFFER_ARB, m_iVBOTCID);
        float *aTexCoords = (float *) glMapBufferARB(GL_ARRAY_BUFFER_ARB, GL_WRITE_ONLY_ARB);

        for(u32 i = 0; i < iCount; i++)
        {
            if(m_kRLT == RLT_LINKED)
            {
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite] = aUpdateData[i].iX;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 1] = aUpdateData[i].iY;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS1);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 1] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT1);

                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 3] = aUpdateData[i].iX + aUpdateData[i].iW;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 4] = aUpdateData[i].iY;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 2] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS2);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 3] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT1);

                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 6] = aUpdateData[i].iX;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 7] = aUpdateData[i].iY + aUpdateData[i].iH;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 4] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS1);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 5] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT2);

                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 9] = aUpdateData[i].iX + aUpdateData[i].iW;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 10] = aUpdateData[i].iY + aUpdateData[i].iH;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 6] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS2);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 7] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT2);
            }
            else if(m_kRLT == RLT_NORMAL)
            {
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite] = aUpdateData[i].iX;
                ...
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 9] = aUpdateData[i].iX;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 10] = aUpdateData[i].iY + aUpdateData[i].iH;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 6] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS1);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 7] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT2);

                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 12] = aUpdateData[i].iX + aUpdateData[i].iW;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 13] = aUpdateData[i].iY + aUpdateData[i].iH;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 8] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS2);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 9] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT2);

                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 15] = aUpdateData[i].iX + aUpdateData[i].iW;
                aVerts[aUpdateData[i].iNum * 3 * iVertsPerSprite + 16] = aUpdateData[i].iY;
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 10] = 1.0f / (m_pTextureData->w / aUpdateData[i].iS2);
                aTexCoords[aUpdateData[i].iNum * 2 * iVertsPerSprite + 11] = 1.0f / (m_pTextureData->h / aUpdateData[i].iT1);
            }
        }
